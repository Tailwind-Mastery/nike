import { Link } from "react-router-dom";
import bhm1 from '../assets/bhm1.jpg'

export default function BHMHome() {
    return (
        <div className="flex flex-col lg:px-10 px-5 lg:gap-10 gap-5">

            <h2 className="md:text-3xl text-xl font-medium">
                2023 BHM Collection
            </h2>
            
            <div className="">
                <img src={bhm1} alt="nike-bhm1" className="w-full object-cover h-96 md:h-[35rem] lg:h-[45rem] xl:h-[50rem] rounded"/>
            </div>
            <div className="flex flex-col gap-5 lg:items-center">
                <h1>
                    <Link to={'/'} className="text-6xl font-bold tracking-tighter">
                        NIKE BHM X PRINCESS AIR
                    </Link>
                </h1>

                <p className="">
                    People love or hate, but Worth something !
                </p>
            </div>

            <div className="flex flex-col md:flex-row gap-2 items-start lg:justify-center">

                <Link to={'/'} className="rounded-full px-5 py-2 bg-black text-white hover:bg-sky-700">
                    Stop Runnin, Be Someone
                </Link>
            </div>
        </div>
    )
}